### MODIFIED VERSION compatible with Python 3
# Changes Victor Loux 25/05/2016
# -> added .encode() on strings
# -> changed import Image to "from PIL import Image"
# -> changes of Inverse in new firmware (see Arduino file update
#    and inverseOn, inverseOff methods)
...
class Adafruit_Thermal(Serial):
    ...
    def inverseOn(self):
        self.writeBytes(29, 65, 1) # 65 = B
        # self.setPrintMode(self.INVERSE_MASK)

    def inverseOff(self):
        self.writeBytes(29, 65, 0) # 65 = B
        ...
        for rowStart in range(0, h, maxChunkHeight):
            ...
            for y in range(chunkHeight):
                for x in range(rowBytesClipped):
                    print(y, x, bitmap[i], chr(bitmap[i]).encode())
